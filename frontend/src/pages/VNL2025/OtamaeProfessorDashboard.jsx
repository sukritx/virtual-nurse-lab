...
const OtamaeProfessorDashboard = () => {
  ...
  const fetchUniversityData = async () => {
    try {
      ...
      const labsResponse = await axios.get('/api/v1/professor/otamae/labs', {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-slate-100">
      ...
      <main className="w-full max-w-7xl px-4 text-center">
        <h1 className="text-4xl font-bold mb-8">Professor Otamae Dashboard</h1>
            ...
            <tbody className="text-gray-600 text-sm font-light">
              {filteredStudents.map((student) => (
                <tr key={student.studentId} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-3 px-6 text-center">
                    <Button onClick={() => navigate(`/professor/otamae/view-labs/${student._id}`)} label="View Labs" />
                  </td>
                </tr>
              ))}
            </tbody>
...
export default OtamaeProfessorDashboard;
