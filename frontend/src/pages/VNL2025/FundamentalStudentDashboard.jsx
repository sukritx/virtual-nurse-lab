...
const FundamentalStudentDashboard = () => {
  ...
  const fetchLabs = async () => {
    try {
      ...
      setLabs(response.data.labs.filter(lab => lab.labInfo.labNumber >= 1 && lab.labInfo.labNumber <= 3));
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="container mx-auto py-10">
        <h1 className="text-3xl font-bold mb-6 text-center">การพยาบาลพื้นฐาน</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {labs.map((lab) => (
            <div key={lab.labInfo._id} className={`bg-white p-4 rounded-lg shadow-md flex flex-col items-center justify-between ${lab.isPass === null ? 'bg-gray-300' : lab.isPass || lab.everPassed ? 'bg-green-300' : 'bg-red-300'}`}>
              ...
              <button
                onClick={() => navigate(`/student/fundamental/${lab.labInfo.labNumber}`)}
              ...
              {lab.isPass !== null && (
                <button
                  onClick={() => navigate(`/fundamental/${lab.labInfo.labNumber}/history`)}
...
export default FundamentalStudentDashboard;
