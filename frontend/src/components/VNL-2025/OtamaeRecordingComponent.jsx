...
const OtamaeRecordingComponent = ({
    labNumber,
    title,
    subtitle,
    description,
    ...
    const onSubmit = useCallback(async () => {
        ...
        try {
            ...
            const fileExt = recordedBlob.type.split(';')[0].split('/')[1];
            const fileName = `otamae-${labNumber}_recording_${Date.now()}.${fileExt}`;
            ...
            const response = await axios.post(`/api/v1/vnl2025/otamae/${labNumber}`, {
                fileName,
                totalChunks,
                language
            }, {
    ...
    }, [recordedBlob, token, attemptsLeft, labNumber, setAttemptsLeft, disableReadyButton, loading, language]);
            ...
            <div className="w-full max-w-2xl p-8">
                ...
                <div className="mb-6 flex justify-center">
                    <select
                        ...
                        className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-full focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300"
                    >
                ...
                {pros && (
                    <div className="mt-6 text-gray-700">
                        <h3 className="text-lg font-bold text-gray-800">良かった点:</h3>
                        <p>{pros}</p>
                    </div>
                )}
                {recommendations && (
                    <div className="mt-6 text-gray-700">
                        <h3 className="text-lg font-bold text-gray-800">改善点:</h3>
                        <p>{recommendations}</p>
                    </div>
                )}
            </div>
...
OtamaeRecordingComponent.propTypes = {
...
OtamaeRecordingComponent.defaultProps = {
    disableReadyButton: false,
};

export default OtamaeRecordingComponent;
