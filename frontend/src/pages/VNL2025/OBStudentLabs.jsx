...
const OBStudentLabs = () => {
  ...
  const fetchStudentLabs = async () => {
    try {
      const response = await axios.get(`/api/v1/professor/student/${userId}/ob/labs`, {
        headers: {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8 w-full">
          {labs.map((lab) => (
            ...
            >
              ...
              {lab.attemptCount > 0 && (
                <button
                  onClick={() => navigate(`/professor/ob/view-lab/${userId}/${lab.labNumber}`)}
