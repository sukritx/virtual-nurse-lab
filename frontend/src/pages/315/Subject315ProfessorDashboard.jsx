...
export const Subject315ProfessorDashboard = () => {
  ...
  const fetchUniversityData = async () => {
    try {
      ...
      const labsResponse = await axios.get('/api/v1/professor/315/labs', {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
            ...
            <tbody className="text-gray-600 text-sm font-light">
              {filteredStudents.map((student) => (
                <tr key={student.studentId} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-3 px-6 text-center">
                    <Button onClick={() => navigate(`/professor/315/view-labs/${student._id}`)} label="View Labs" />
                  </td>
                </tr>
              ))}
            </tbody>
