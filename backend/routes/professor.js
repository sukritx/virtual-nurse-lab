...
router.get('/download-scores', professorAuth, async (req, res) => {
  try {
    ...
    if (professor.universityCode) {
      ...
      } else if (upperUniversityCode.startsWith("FUN")) {
        subjectFilter = { subject: "fundamental" };
      } else if (upperUniversityCode.startsWith("OTA")) {
        subjectFilter = { subject: "otamae" };
      }
...
// get students' Fundamental lab info
router.get('/fundamental/labs', professorAuth, async (req, res) => {
  try {
    const university = await University.findOne({ professor: { $in: [req.userId] } })
      .populate('students', '_id firstName lastName studentId')
      .select('students');
    ...
    const allLabs = await LabInfo.find({ subject: 'fundamental' }).sort({ labNumber: 1 }).exec();
...
// Get all labs of FUNDAMENTAL for a specific student
router.get('/student/:userId/fundamental/labs', professorAuth, async (req, res) => {
  try {
    const student = await User.findOne({ _id: req.params.userId });
    if (!student) {
      return res.status(404).json({ message: 'Student not found' });
    ...
    const labInfos = await LabInfo.find({ subject: 'fundamental' }).select('labNumber');
    const labSubmissions = await LabSubmission.aggregate([
      { $match: { studentId: student._id } },
      { $sort: { timestamp: -1 } },
      {
...
// Get details of a specific lab of FUNDAMENTAL for a specific student
router.get('/student/:userId/lab/fundamental/:labNumber', professorAuth, async (req, res) => {
  try {
    const student = await User.findOne({ _id: req.params.userId });
    if (!student) {
      return res.status(404).json({ message: 'Student not found' });
    ...
    const labInfo = await LabInfo.findOne({ labNumber: req.params.labNumber, subject: 'fundamental' });
    if (!labInfo) {
      return res.status(404).json({ message: 'Lab not found' });
    }
...
// get students' Otamae lab info
router.get('/otamae/labs', professorAuth, async (req, res) => {
  try {
    const university = await University.findOne({ professor: { $in: [req.userId] } })
      .populate('students', '_id firstName lastName studentId')
      .select('students');
    ...
    const allLabs = await LabInfo.find({ subject: 'otamae' }).sort({ labNumber: 1 }).exec();
...
// Get all labs of OTAMAE for a specific student
router.get('/student/:userId/otamae/labs', professorAuth, async (req, res) => {
  try {
    const student = await User.findOne({ _id: req.params.userId });
    if (!student) {
      return res.status(404).json({ message: 'Student not found' });
    ...
    const labInfos = await LabInfo.find({ subject: 'otamae' }).select('labNumber');
    const labSubmissions = await LabSubmission.aggregate([
      { $match: { studentId: student._id } },
      { $sort: { timestamp: -1 } },
      {
...
// Get details of a specific lab of OTAMAE for a specific student
router.get('/student/:userId/lab/otamae/:labNumber', professorAuth, async (req, res) => {
  try {
    const student = await User.findOne({ _id: req.params.userId });
    if (!student) {
      return res.status(404).json({ message: 'Student not found' });
    ...
    const labInfo = await LabInfo.findOne({ labNumber: req.params.labNumber, subject: 'otamae' });
    if (!labInfo) {
      return res.status(404).json({ message: 'Lab not found' });
    }
