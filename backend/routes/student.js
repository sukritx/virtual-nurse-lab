...
// Surgical nursing
router.get('/surgical/labs', authMiddleware, async (req, res) => {
    try {
        const studentId = req.userId;
        const labSubmissions = await LabSubmission.find({ studentId }).populate('labInfo').exec();
        const allLabs = await LabInfo.find({ subject: 'surgical' }).sort({ labNumber: 1 });
...
// Medical nursing
router.get('/medical/labs', authMiddleware, async (req, res) => {
    try {
        const studentId = req.userId;
        const labSubmissions = await LabSubmission.find({ studentId }).populate('labInfo').exec();
        const allLabs = await LabInfo.find({ subject: 'medical' }).sort({ labNumber: 1 });
...
// OB nursing
router.get('/ob/labs', authMiddleware, async (req, res) => {
    try {
        const studentId = req.userId;
        const labSubmissions = await LabSubmission.find({ studentId }).populate('labInfo').exec();
        const allLabs = await LabInfo.find({ subject: 'ob' }).sort({ labNumber: 1 });
...
router.get('/fundamental/labs', authMiddleware, async (req, res) => {
    try {
        const studentId = req.userId;
        const labSubmissions = await LabSubmission.find({ studentId }).populate('labInfo').exec();
        const allLabs = await LabInfo.find({ subject: 'fundamental' }).sort({ labNumber: 1 });
...
// Otamae nursing
router.get('/otamae/labs', authMiddleware, async (req, res) => {
    try {
        const studentId = req.userId;
        const labSubmissions = await LabSubmission.find({ studentId }).populate('labInfo').exec();
        const allLabs = await LabInfo.find({ subject: 'otamae' }).sort({ labNumber: 1 });
